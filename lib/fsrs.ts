...
// Short-term steps (in minutes) for cards still in learning / relearning.
...
export function schedule(
  ...
  // First-ever review — initialise stability and difficulty.
  if (state === "NEW") {
  ...
  if (state === "NEW" || state === "LEARNING") {
    if (rating === 4) {
      ...
      if (rating === 3) {
        // Good in learning → graduate after the step.
        state = "REVIEW";
        scheduledDays = intervalFromStability(stability);
        due = addDays(now, scheduledDays);
      }
...
// Preview the resulting interval for each rating without persisting.
export function previewIntervals(card: CardSnapshot, now: Date = new Date()) {
